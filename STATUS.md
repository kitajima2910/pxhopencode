...
| 2026-07-27 | v82.4 | **start.bat + init-memory.ps1 fix** — thêm `start.bat` để chạy init từ cmd/double-click. Fix parser bug PSv5.1 với `'\S'` trong regex patterns. Step 2 nâng cấp: merge toàn bộ entries từ template `.gitignore` (15 entries) vào parent project. Tested: 13 memory files + 15 gitignore entries OK. |
