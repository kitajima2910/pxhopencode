...
10 AI agents, 4-tier runtime, 8 workflows, 30 skills, 162 templates.
...
| `pxh-mod-apk` | T3 | Reverse engineering, patch smali/dex, repack APK/XAPK |
| `pxh-help` | T1 | Hướng dẫn workflow |

## Runtime 4 Tầng

T1 (Interface) → Request → T2 (Orchestration) → Task → T3 (Workers: 7 agents) → Result → T2 → Response → T1. Mọi tầng → Event → T4 (Infrastructure). Giao tiếp qua 6 contract: Request, Task, Result, Response, Event, State. Policy: Retry (exp backoff max 3), Recovery (checkpoint), Reflection (4 mức).
...
- Lệnh `/`: `/vibe` `/web` `/game` `/ai` `/tool` `/debug` `/release` `/meeting` `/mod`
